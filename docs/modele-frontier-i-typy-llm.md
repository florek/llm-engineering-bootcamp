...
### Mechanika gry wieloagentowej jako benchmark

W demo kursu cztery modele rywalizują w grze ekonomicznej o stały kapitał (np. po 12 jednostek na start). W każdej turze gracz:

1. wymienia wiadomości z pozostałymi (negocjacje, propozycje sojuszy),
2. przenosi jedną jednostkę od wybranego gracza do innego,
3. uzasadnia strategię i ujawnia wewnętrzne rozumowanie.

**Sojusz** powstaje, gdy dwóch graczy wspólnie atakuje tego samego przeciwnika i wzajemnie przekazuje sobie jednostki — wtedy otrzymują bonus i dodatkowo zabierają zasoby ofierze. Obserwacja wiadomości między agentami oraz ich strategii pozwala porównać styl komunikacji (dyplomacja, schematy, wiarygodność obietnic) i jakość planowania między modelami.

W publicznych demach często używa się tańszych, szybszych modeli ze względu na koszt API i ruch użytkowników; lokalnie można testować najnowsze flagshipi bez limitu publicznego dostępu. Platformy inference zoptymalizowane pod szybkość (np. Groq) mogą uruchamiać modele open-source znacznie szybciej niż lokalny sprzęt — to ważny czynnik przy porównaniach latency.
